...
train_df = utils.extract_datefields(train_df).drop(columns = ['argument_date'])
eval_df = utils.extract_datefields(eval_df).drop(columns = ['argument_date'])
test_df = utils.extract_datefields(test_df).drop(columns = ['argument_date'])
...
scores.to_csv("Additional Details/KBestScores_Main.csv", index=False)
...
# Train models
best_classifiers = utils.create_best_classifiers("Main", models, param_grid, X_scaled_train, y_scaled_train)
...
importance_df.to_csv("Additional Details/RandomForestBestFeatures_Main.csv", index=False)
...
utils.predict_test(best_classifiers, test_df_id, X_scaled_test, "Main", record=True)